...
import { lossesAndCreditsMachine } from '@/machines/lossesAndCreditsMachine';
import { LossesAndCreditsContext, isS207Exempt } from '@/machines/deriveLossesAndCredits';
...
export default function LossesAndCreditsStep({ initialContext, onBack, onContinue, onAutoSave }: LossesAndCreditsStepProps) {
      ...
      <div className="p-5 bg-white/5 border border-white/5 rounded-2xl flex flex-col gap-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-white/5 pb-3">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <button
              onClick={() => (send as any)({ type: 'TOGGLE_BF_LOSSES', value: true })}
            ...
            <button
              onClick={() => (send as any)({ type: 'TOGGLE_BF_LOSSES', value: false })}
          ...
          <div id="div-s79-loss" className="flex flex-col md:flex-row md:items-center justify-between gap-3 border border-brandRed/20 bg-brandRed/5 p-3 rounded-xl mb-2">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              <label className="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" checked={!!context.s79_shareholding_change} onChange={(e) => (send as any)({ type: 'SET_S79_CHANGE', value: e.target.checked })} className="sr-only peer" />
        ...
        {context.has_brought_forward_losses === true && (
          <div id="div-loss-table-container" className="flex flex-col gap-4">
            <div className="flex justify-between items-center bg-white/5 p-3 rounded-xl border border-white/5">
              ...
              <button onClick={() => (send as any)({ type: 'ADD_LOSS_ROW', category: 'business_loss_cf' })} className="px-3 py-1 bg-brandGold/20 hover:bg-brandGold/30 text-brandGold rounded text-[9px] font-black uppercase tracking-widest transition-all font-sans font-bold">+ Add Loss Card</button>
            </div>
                ...
                return list.map(item => (
                  <div key={item.id} className="loss-card bg-black/30 border border-white/10 rounded-2xl p-4 flex flex-col gap-3">
                    <div className="flex justify-between items-center">
                      ...
                      <button onClick={() => (send as any)({ type: 'REMOVE_LOSS_ROW', category: cat, id: item.id })} className="text-brandRed/70 hover:text-brandRed text-[9px] font-black uppercase tracking-widest transition-all">✕ Remove</button>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      <div>
                        ...
                        <select
                          ...
                          value={cat}
                          onChange={(e) => {
                            (send as any)({ type: 'REMOVE_LOSS_ROW', category: cat, id: item.id });
                            (send as any)({ type: 'ADD_LOSS_ROW', category: e.target.value });
                          }}
                        >
                    ...
                    {cat !== 'house_property_loss_cf' && (
                      <div className="loss-filing-gate flex items-center justify-between p-3 bg-white/5 border border-white/10 rounded-xl">
                        ...
                        <label className="relative inline-flex items-center cursor-pointer">
                          <input type="checkbox" checked={item.was_filed_before_due_date} onChange={(e) => (send as any)({ type: 'UPDATE_LOSS_ROW', category: cat, id: item.id, field: 'was_filed_before_due_date', value: e.target.checked })} className="sr-only peer" />
      ...
      <div className="p-5 bg-white/5 border border-white/5 rounded-2xl flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input type="number" placeholder="0" value={context.unabsorbed_depreciation_cf || ''} onChange={(e) => (send as any)({ type: 'UPDATE_UNABSORBED_DEP', field: 'unabsorbed_depreciation_cf', value: parseFloat(e.target.value) || null })} className="inr-input w-full bg-white/5 border border-white/10 rounded-xl text-xs text-white px-3 py-2 focus:border-brandGold focus:ring-0 font-mono" />
          ...
          {context.tax_regime === 'NEW' && (
            <div>
              ...
              <input type="number" placeholder="0" value={context.unabsorbed_depreciation_attributable_to_additional_dep_inr || ''} onChange={(e) => (send as any)({ type: 'UPDATE_UNABSORBED_DEP', field: 'unabsorbed_depreciation_attributable_to_additional_dep_inr', value: parseFloat(e.target.value) || null })} className="inr-input w-full bg-white/5 border border-white/10 rounded-xl text-xs text-white px-3 py-2 focus:border-brandGold focus:ring-0 font-mono" />
      ...
      <div className="p-5 bg-white/5 border border-white/5 rounded-2xl flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 border-b border-white/5 pb-4">
          <div>
            ...
            <input type="number" placeholder="0" value={context.tds_already_deducted_inr || ''} onChange={(e) => (send as any)({ type: 'UPDATE_CREDIT', field: 'tds_already_deducted_inr', value: parseFloat(e.target.value) || null })} className="inr-input w-full bg-white/5 border border-white/10 rounded-xl text-xs text-white px-3 py-2 focus:border-brandGold focus:ring-0 font-mono" />
          </div>
          ...
          <div className="flex items-center justify-between p-3 bg-white/5 border border-white/10 rounded-xl">
            ...
            <label className="relative inline-flex items-center cursor-pointer">
              <input type="checkbox" checked={context.form_26as_uploaded} onChange={(e) => (send as any)({ type: 'TOGGLE_26AS', value: e.target.checked })} className="sr-only peer" />
              <div className="w-9 h-5 bg-white/10 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-zinc-400 after:border-zinc-300 peer-checked:bg-brandGold peer-checked:after:bg-black"></div>
            </label>
          </div>
          ...
          <div className="flex items-center justify-between mb-1">
            ...
            <label className="flex items-center gap-2 cursor-pointer group">
              ...
              <div className="relative inline-flex items-center">
                <input type="checkbox" checked={context.simulator_active} onChange={(e) => (send as any)({ type: 'TOGGLE_SIMULATOR', value: e.target.checked })} className="sr-only peer" />
            ...
            <div className="flex flex-col gap-3 p-3 bg-brandGold/5 border border-brandGold/20 rounded-xl mb-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
                <div>
                  ...
                  <input type="number" placeholder="₹0" value={context.sim_salary || ''} onChange={(e) => (send as any)({ type: 'UPDATE_SIM_FIELD', field: 'sim_salary', value: parseFloat(e.target.value) || null })} className="inr-input w-full bg-white/5 border border-white/10 rounded-xl text-xs text-white px-3 py-2 focus:border-brandGold focus:ring-0 font-mono" />
                </div>
                <div>
                  <label className="block text-[9px] font-black uppercase tracking-widest text-white/40 mb-1">Projected Total Biz Profit</label>
                  <input type="number" placeholder="₹0" value={context.sim_business || ''} onChange={(e) => (send as any)({ type: 'UPDATE_SIM_FIELD', field: 'sim_business', value: parseFloat(e.target.value) || null })} className="inr-input w-full bg-white/5 border border-white/10 rounded-xl text-xs text-white px-3 py-2 focus:border-brandGold focus:ring-0 font-mono" />
                </div>
                <div>
                  ...
                  <div className="flex flex-col xl:flex-row gap-1">
                    <input type="number" placeholder="₹0" value={context.sim_cg || ''} onChange={(e) => (send as any)({ type: 'UPDATE_SIM_FIELD', field: 'sim_cg', value: parseFloat(e.target.value) || null })} className="inr-input w-full xl:w-2/3 bg-white/5 border border-white/10 rounded-xl text-xs text-white px-3 py-2 focus:border-brandGold focus:ring-0 font-mono" />
                    <select value={context.sim_cg_quarter} onChange={(e) => (send as any)({ type: 'UPDATE_SIM_FIELD', field: 'sim_cg_quarter', value: e.target.value })} className="w-full xl:w-1/3 bg-white/5 border border-white/10 rounded-xl text-xs text-white px-2 py-2 focus:border-brandGold focus:ring-0">
                ...
                <div>
                  ...
                  <input type="number" placeholder="₹0" value={context.sim_other || ''} onChange={(e) => (send as any)({ type: 'UPDATE_SIM_FIELD', field: 'sim_other', value: parseFloat(e.target.value) || null })} className="inr-input w-full bg-white/5 border border-white/10 rounded-xl text-xs text-white px-3 py-2 focus:border-brandGold focus:ring-0 font-mono" />
                </div>
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              <div className="flex justify-between items-end mb-1">
                <label className="block text-[9px] font-black uppercase tracking-widest text-white/40">Q1 (By 15 June {startYear}) — 15%</label>
                <span className="text-[9px] font-bold text-brandGold font-mono">Target: ₹0</span>
              </div>
              <input type="number" placeholder="Amount Paid" value={context.advance_tax_q1_15jun_inr || ''} onChange={(e) => (send as any)({ type: 'UPDATE_CREDIT', field: 'advance_tax_q1_15jun_inr', value: parseFloat(e.target.value) || null })} className="inr-input w-full bg-white/5 border border-white/10 rounded-xl text-xs text-white px-3 py-2 focus:border-brandGold focus:ring-0 font-mono" />
            </div>
            <div>
              <div className="flex justify-between items-end mb-1">
                <label className="block text-[9px] font-black uppercase tracking-widest text-white/40">Q2 (By 15 Sept {startYear}) — 45%</label>
                <span className="text-[9px] font-bold text-brandGold font-mono">Target: ₹0</span>
              </div>
              <input type="number" placeholder="Amount Paid" value={context.advance_tax_q2_15sep_inr || ''} onChange={(e) => (send as any)({ type: 'UPDATE_CREDIT', field: 'advance_tax_q2_15sep_inr', value: parseFloat(e.target.value) || null })} className="inr-input w-full bg-white/5 border border-white/10 rounded-xl text-xs text-white px-3 py-2 focus:border-brandGold focus:ring-0 font-mono" />
            </div>
            <div>
              <div className="flex justify-between items-end mb-1">
                <label className="block text-[9px] font-black uppercase tracking-widest text-white/40">Q3 (By 15 Dec {startYear}) — 75%</label>
                <span className="text-[9px] font-bold text-brandGold font-mono">Target: ₹0</span>
              </div>
              <input type="number" placeholder="Amount Paid" value={context.advance_tax_q3_15dec_inr || ''} onChange={(e) => (send as any)({ type: 'UPDATE_CREDIT', field: 'advance_tax_q3_15dec_inr', value: parseFloat(e.target.value) || null })} className="inr-input w-full bg-white/5 border border-white/10 rounded-xl text-xs text-white px-3 py-2 focus:border-brandGold focus:ring-0 font-mono" />
            </div>
            <div>
              <div className="flex justify-between items-end mb-1">
                <label className="block text-[9px] font-black uppercase tracking-widest text-white/40">Q4 (By 15 Mar {endYear}) — 100%</label>
                <span className="text-[9px] font-bold text-brandGold font-mono">Target: ₹0</span>
              </div>
              <input type="number" placeholder="Amount Paid" value={context.advance_tax_q4_15mar_inr || ''} onChange={(e) => (send as any)({ type: 'UPDATE_CREDIT', field: 'advance_tax_q4_15mar_inr', value: parseFloat(e.target.value) || null })} className="inr-input w-full bg-white/5 border border-white/10 rounded-xl text-xs text-white px-3 py-2 focus:border-brandGold focus:ring-0 font-mono" />
            </div>
          </div>
        </div>
      </div>
