...
//        residency-alert panel — see residencyMachine.ts). Both are
...
export const US_TREATY_AUTOFILL: Record<string, { elected_rate: number | null, treaty_article: string | null }> = {
...
export interface DtaaElection {
  income_type: string | null;
  amount_inr: number | null;
  elected_rate: number | null;
  treaty_article: string | null;
  banner?: { type: string, text: string } | null;
}

export function defaultElection(): DtaaElection {
...
export function applyElectionUpdate(election: DtaaElection, field: string, val: any): DtaaElection {
  let next = { ...election } as any;
...
export function computeIsUsResident(countryCode: string | null): boolean | null {
  return countryCode === 'US' ? true : null;
}

// Per-row warning/info banner, ported from renderDtaaElections()'s inline
// income_type checks.
export function computeElectionBanner(election: DtaaElection): { type: string, text: string } | null {
...
export function computeStepEligibility(context: any) {
...
export function computeVisibility(context: any) {
  ...
  const dtaa = context?.dtaa || {};
  return {
    stepEligible: eligibility.eligible,
    eligibility,
    divTrcUpload: !!dtaa.trc_status,
    divPeAlert: !!dtaa.has_permanent_establishment_in_india,
    ...
    elections: (dtaa.treaty_elections || []).map((e: DtaaElection) => ({ ...e, banner: computeElectionBanner(e) })),
  };
}

export function deriveAll(rawContext: any) {
