      ...
      <div className="max-w-[1440px] mx-auto px-4 lg:px-8 h-20 flex items-center justify-between">
        <div className="flex items-center space-x-4 lg:space-x-12">
          ...
          >
            ...
            {/* Sidebar Menu */}
            <div 
              className={`fixed left-0 top-20 bottom-0 w-80 bg-black transition-all duration-300 z-50 border-r border-white/5 ${isSidebarOpen ? 'opacity-100 visible pointer-events-auto' : 'opacity-0 invisible pointer-events-none'}`}
            >
              <div className="flex flex-col text-[11px] font-bold tracking-[0.15em] uppercase text-white/50 pt-6">
                <Link to="/" className="flex items-center px-10 py-5 hover:bg-[#111] hover:text-white border-l-4 border-transparent hover:border-white transition-colors">
                  Tax Intelligence
                </Link>
                <Link to="/income" className="flex items-center px-10 py-5 hover:bg-white/5 transition-colors">
                  Salaries & RSUs
                </Link>
                <Link to="/router" className="flex items-center px-10 py-5 hover:bg-white/5 transition-colors">
                  Jurisdiction Router
                </Link>
                <Link to="/layer1-india" className="flex items-center px-10 py-5 hover:bg-white/5 transition-colors">
                  Layer 1 India
                </Link>
              </div>
            </div>
        ...
        <div className="flex items-center space-x-4 lg:space-x-8">
          <div className="hidden sm:flex items-center space-x-4">
            ...
            <span className="text-[10px] font-black text-white/40 uppercase tracking-widest">Global Node: Active</span>
          </div>
          <div className="hidden sm:block w-px h-6 bg-white/10"></div>
          <button onClick={() => window.print()} className="w-8 h-8 rounded-full border border-white/10 flex items-center justify-center text-white/50 hover:text-white transition-colors bg-[#080808] hover:bg-white/5 relative group">
            ...
            <span className="absolute top-full mt-2 right-0 bg-black border border-white/10 px-2 py-1 rounded text-[8px] font-bold uppercase opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">CPA Audit Export</span>
          </button>
