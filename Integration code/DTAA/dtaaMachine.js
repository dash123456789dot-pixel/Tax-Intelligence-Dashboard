...
// requested) hold — see deriveDtaa.js -> computeStepEligibility().
// ────────────────────────────────────────────────────────────────────────

import { setup, assign } from 'xstate';
import { deriveAll, defaultElection, applyElectionUpdate, autoFillUSTreaty, computeIsUsResident } from './deriveDtaa.js';
...
export const initialDtaaContext = {
  ...
  ui: {}, // fully derived — see deriveDtaa.js -> deriveAll()
};
...
export const dtaaMachine = setup({
  ...
  id: 'dtaaStep',
  context: ({ input }) => deriveAll({ ...initialDtaaContext, ...(input || {}) }),
  initial: 'ready',
  states: {
    ready: {
      on: {
        ...
        SET_DAYS_IN_INDIA: {
          actions: [assign(({ context, event }) => ({ ...context, days_in_india_current_year: event.value })), 'recomputeDerived'],
        },
        ...
        TOGGLE_TRC_STATUS: {
          actions: [assign(({ context, event }) => ({ ...context, dtaa: { ...context.dtaa, trc_status: !!event.value } })), 'recomputeDerived'],
        },

        TOGGLE_PERMANENT_ESTABLISHMENT: {
          actions: [
            assign(({ context, event }) => ({ ...context, dtaa: { ...context.dtaa, has_permanent_establishment_in_india: !!event.value } })),
            'recomputeDerived',
          ],
        },
        ...
        ADD_ELECTION: {
          actions: [
            assign(({ context }) => ({ ...context, dtaa: { ...context.dtaa, treaty_elections: [...context.dtaa.treaty_elections, defaultElection()] } })),
            'recomputeDerived',
          ],
        },
        REMOVE_ELECTION: {
          actions: [
            assign(({ context, event }) => ({ ...context, dtaa: { ...context.dtaa, treaty_elections: context.dtaa.treaty_elections.filter((_, i) => i !== event.index) } })),
            'recomputeDerived',
          ],
        },
        UPDATE_ELECTION: {
          actions: [
            assign(({ context, event }) => ({
              ...
              dtaa: {
                ...context.dtaa,
                treaty_elections: context.dtaa.treaty_elections.map((el, i) => (i === event.index ? applyElectionUpdate(el, event.field, event.value) : el)),
              },
            })),
            'recomputeDerived',
          ],
        ...
        AUTOFILL_US_TREATY: {
          actions: [assign(({ context }) => ({ ...context, dtaa: { ...context.dtaa, treaty_elections: autoFillUSTreaty() } })), 'recomputeDerived'],
        },
