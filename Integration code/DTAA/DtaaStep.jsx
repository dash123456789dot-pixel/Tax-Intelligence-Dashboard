// ────────────────────────────────────────────────────────────────────────
// DtaaStep.jsx
// React port of panel-step-dtaa ("Screen 2B — Tax Treaties (DTAA)") from
// layer1_india.html. See dtaaMachine.js's header comment for the full
// cross-step dependency map and the combined NR + days<180 eligibility
// gate this step now enforces (per explicit instruction).
//
// The "⚡ Auto-Fill US Rates" button is rendered but always hidden,
// matching the original exactly — its `hidden` class was never removed by
// any code path in the source (dead UI; see deriveDtaa.js's comment). The
// wired `autoFillUSTreaty()` logic itself is fully implemented and
// reachable via the AUTOFILL_US_TREATY event if a parent wants to surface
// it through some other control.
// ────────────────────────────────────────────────────────────────────────

import React from 'react';
import { useMachine } from '@xstate/react';
import { dtaaMachine } from '../dtaaMachine.js';
import { formatINRDisplay } from '../deriveDtaa.js';
...
function ElectionRow({ election, idx, send }) {
  const update = (field) => (value) => send({ type: 'UPDATE_ELECTION', index: idx, field, value });
  return (
    <>
      <tr className="border-b border-white/5">
        <td className="p-2">
          <select
            ...
            className="w-full bg-[#121212] border border-white/10 rounded-lg text-xs text-white px-2 py-2 focus:border-brandGold focus:ring-0 focus:outline-none transition-all"
          >
        ...
        <td className="p-2 text-right">
          <button
            onClick={() => send({ type: 'REMOVE_ELECTION', index: idx })}
...
export default function DtaaStep({ initialContext, onBack, onContinue }) {
  const [state, send] = useMachine(dtaaMachine, { input: initialContext });
    ...
    <div id="panel-step-dtaa" className="glass-card p-6 lg:p-8 flex flex-col gap-6">
      ...
      {!v.stepEligible && (
        <div className="p-4 bg-white/5 border border-white/10 rounded-2xl text-[11px] text-white/50 leading-relaxed">
          This step is only relevant if your derived residency status is <strong className="text-white/70">Non-Resident (NR)</strong> and you
          spent fewer than <strong className="text-white/70">180 days in India</strong> this Tax Year (per the Residency Solver step).
          {' '}Currently: status = <span className="text-brandGold">{ctx.final_india_residency_status}</span>, days in India ={' '}
          <span className="text-brandGold">{ctx.days_in_india_current_year}</span>.
        </div>
      )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="col-span-1 flex flex-col gap-2">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => send({ type: 'SET_TAX_RESIDENCY_COUNTRY', value: e.target.value })}
                  className="w-full bg-[#121212] border border-white/10 rounded-xl text-xs text-white px-4 py-3 focus:border-brandGold focus:ring-0 focus:outline-none transition-all"
                >
            ...
            <div className="p-4 bg-white/5 border border-white/10 rounded-2xl md:col-span-2">
              <div className="flex items-center justify-between">
                ...
                <ToggleSwitch checked={dtaa.trc_status} onChange={(val) => send({ type: 'TOGGLE_TRC_STATUS', value: val })} />
              </div>
                ...
                <div id="div-trc-upload" className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4 pt-4 border-t border-white/10">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => send({ type: 'UPDATE_COMP_TRC', field: 'document_uploaded', value: e.target.files.length > 0 })}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => send({ type: 'UPDATE_COMP_TRC', field: 'validity_start_date', value: e.target.value })}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => send({ type: 'UPDATE_COMP_TRC', field: 'validity_end_date', value: e.target.value })}
                      className="w-full bg-[#121212] border border-white/10 rounded-xl text-xs text-white px-3 py-2.5 focus:border-brandGold focus:ring-0 focus:outline-none transition-all font-mono"
                    />
                  </div>
                </div>
            ...
            <div className="md:col-span-2">
              <div className="flex items-center justify-between p-4 bg-white/5 border border-white/10 rounded-2xl">
                ...
                <ToggleSwitch checked={dtaa.has_permanent_establishment_in_india} onChange={(val) => send({ type: 'TOGGLE_PERMANENT_ESTABLISHMENT', value: val })} />
              </div>
              {v.divPeAlert && (
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex gap-2">
                {v.showUsAutofillBtn && (
                  <button onClick={() => send({ type: 'AUTOFILL_US_TREATY' })} className="px-3 py-1 bg-brandGold/10 hover:bg-brandGold/30 text-brandGold rounded text-[9px] font-black uppercase tracking-widest transition-all">
                    ⚡ Auto-Fill US Rates
                  </button>
                )}
                <button onClick={() => send({ type: 'ADD_ELECTION' })} className="px-3 py-1 bg-white/10 hover:bg-brandGold/20 hover:text-brandGold rounded text-[9px] font-black uppercase tracking-widest transition-all">
                  + Add Stream
                </button>
              </div>
            </div>
              ...
              <table className="w-full text-left text-xs text-white/80" id="dtaa-table">
                ...
                <tbody>
                  {dtaa.treaty_elections.length === 0 ? (
                    ...
                    v.elections.map((election, idx) => <ElectionRow key={idx} election={election} idx={idx} send={send} />)
                  )}
                </tbody>
              </table>
            </div>
