...
// - Every onchange/oninput/onclick handler becomes an XState `send`.
// - `.sal-old-regime-field` visibility (a class-based DOM query in the
//   original) becomes a straight `{ui.visibility.showOldRegimeFields && ...}`
//   read, driven by the injected `tax_regime` context field.
// - The `.inr-input` live-formatting behavior (a global `document`-level
//   input listener in the original) is reproduced locally per-field via
//   formatINRDisplay(), since React fields are controlled.
// ────────────────────────────────────────────────────────────────────────

import React, { useState } from 'react';
import { useMachine } from '@xstate/react';
import { salaryMachine } from '../salaryMachine.js';
import { formatINRDisplay } from '../deriveSalary.js';
...
function InrField({ id, label, tooltip, value, onCommit, className = '', inputClassName = '' }) {
  // Controlled local text buffer so the person can type freely; commits a
  // parsed number up to the machine on every keystroke (mirrors
  // updateSalaryNum/updateSalaryNestedNum being called on `oninput`).
...
export default function SalaryStep({ initialContext, onBack, onContinue }) {
  const [state, send] = useMachine(salaryMachine, { input: initialContext });
  const ctx = state.context;
  const { salary: sal, ui } = ctx;
  const { visibility: v } = ui;

  const updateNum = (field) => (raw) => send({ type: 'UPDATE_SALARY_NUM', field, value: raw });
  const updateNestedNum = (objName, field) => (raw) => send({ type: 'UPDATE_SALARY_NESTED_NUM', objName, field, value: raw });
            ...
            <div className="p-5 bg-brandGold/10 border border-brandGold/30 rounded-2xl flex flex-col md:flex-row items-center justify-between gap-4">
              <div className="flex flex-col">
                ...
                <span className="text-[10px] text-white/60 mt-1">
                  Upload your Form 130 (Part B) PDF to instantly extract gross salary, perquisites, allowances, and NPS
                  contributions.
                </span>
              </div>
