...
// Chapter VI-A") from layer1_india.html.
//
// IMPORTANT: this step is gated behind the OLD tax regime — see
// deductionsMachine.js's header comment for the correction to the "always
// visible" premise, verified directly against the source's own
// `syncUnlockStatus()`. When `context.tax_regime !== 'OLD'`, this
// component renders an explanatory banner instead of the form.
// ────────────────────────────────────────────────────────────────────────

import React, { useState, useEffect } from 'react';
import { useMachine } from '@xstate/react';
import { deductionsMachine } from '../deductionsMachine.js';
import { formatINRDisplay } from '../deriveDeductions.js';
...
function DisabilityCard({ title, blocked, checked, onCheckedChange, showDetails, children }) {
  return (
...
function DonationRow({ row, idx, visibility, send }) {
  const update = (field) => (value) => send({ type: 'UPDATE_80G_ROW', index: idx, field, value });
  return (
    <div className="g80-card bg-black/30 border border-white/10 rounded-2xl p-4 flex flex-col gap-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div>
          ...
          <InrField value={row.donation_amount_raw} onCommit={update('donation_amount_raw')} className="bg-white/5" />
        </div>
        <div>
        ...
        <div className="md:col-span-2 flex items-center justify-between p-3 bg-white/5 border border-white/10 rounded-xl">
          ...
          <ToggleSwitch checked={row.subject_to_qualifying_limit} onChange={update('subject_to_qualifying_limit')} />
        </div>
...
export default function DeductionsStep({ initialContext, onBack, onContinue }) {
      ...
      {v.stepEligible && (
        <>
          ...
          <DisabilityCard
            ...
            onCheckedChange={(val) => send({ type: 'UPDATE_DD_BOOL', section: 's80DD', field: 'has_disabled_dependents', value: val })}
          ...
          <DisabilityCard
            ...
            onCheckedChange={(val) => send({ type: 'UPDATE_DD_BOOL', section: 's80DDB', field: 'has_specified_diseases_treatment', value: val })}
          ...
          <DisabilityCard
            ...
            onCheckedChange={(val) => send({ type: 'UPDATE_DD_BOOL', section: 's80U', field: 'has_self_disability', value: val })}
