...
// See deriveOtherSources.js's header comment for exactly what each gates.
// ────────────────────────────────────────────────────────────────────────

import { setup, assign } from 'xstate';
import { deriveAll, applyOSFieldUpdate, clearFieldsForEntityChange, parseINRCurrency } from './deriveOtherSources.js';
...
export const otherSourcesMachine = setup({
  ...
  id: 'otherSourcesStep',
  context: ({ input }) => deriveAll({ ...initialOtherSourcesContext, ...(input || {}) }),
  initial: 'ready',
  states: {
    ready: {
      on: {
        ...
        SET_DEEMED_DIVIDEND_FROM_BUYBACK: {
          actions: [assign(({ context, event }) => ({ ...context, deemed_dividend_from_buyback_inr: event.value })), 'recomputeDerived'],
        },
