// ────────────────────────────────────────────────────────────────────────
// OtherSourcesStep.jsx
// React port of panel-step-os ("Screen 2I — Other Sources") from
// layer1_india.html. See otherSourcesMachine.js's header comment for the
// cross-step dependency map (entity_type from Step 1, deemed_dividend_
// from_buyback_inr computed by the Capital Gains step).
//
// Two small collapsible "+ Add TDS" sub-fields per income line are
// component-local UI state (not domain state) — exactly like the
// original's `classList.toggle('hidden')` on a plain onclick with no state
// write. The two gift-exemption checkboxes (os-gifts-marriage /
// os-gifts-relative) are rendered but intentionally NOT wired to
// anything, because they had no `onchange` handler in the original either
// — a real, faithfully-preserved dead control, not an oversight here.
// ────────────────────────────────────────────────────────────────────────

import React, { useState, useEffect } from 'react';
import { useMachine } from '@xstate/react';
import { otherSourcesMachine } from '../otherSourcesMachine.js';
import { formatINRDisplay } from '../deriveOtherSources.js';
...
function InrField({ id, value, onCommit, className = '', placeholder = '₹ 0' }) {
...
function FieldWithTds({ label, subtitle, fieldId, value, onCommit, tdsLabel, tdsNote, tdsValue, onTdsCommit }) {
...
export default function OtherSourcesStep({ initialContext, onBack, onContinue }) {
  const [state, send] = useMachine(otherSourcesMachine, { input: initialContext });
  const ctx = state.context;
  const { other_sources: os, agricultural: ag, ui } = ctx;
  const { visibility: v } = ui;

  const update = (field) => (raw) => send({ type: 'UPDATE_OS_FIELD', field, value: raw });
  ...
  return (
    <div id="panel-step-os" className="glass-card p-6 lg:p-8 flex flex-col gap-6">
      ...
      <div className="p-5 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-between">
        ...
        <ToggleSwitch checked={os.has_other_sources_income} onChange={(val) => send({ type: 'TOGGLE_OS_SECTION', value: val })} />
      </div>
          ...
          <div className="p-5 bg-[#161616] border border-white/10 rounded-2xl relative overflow-hidden group">
            ...
            <div className="p-5 bg-white/5 border border-white/5 rounded-2xl flex flex-col gap-4 mt-6">
              <div className="flex items-center justify-between">
                ...
                <ToggleSwitch size="sm" checked={ag.has_agricultural_income} onChange={(val) => send({ type: 'TOGGLE_AGRI_SECTION', value: val })} />
              </div>
              {ag.has_agricultural_income && (
                <div id="div-agri-section">
                  ...
                  <InrField id="ag-income" value={ag.agricultural_income_inr} onCommit={(v2) => send({ type: 'UPDATE_AGRI_INCOME', value: v2 })} className="max-w-sm" />
                </div>
              )}
            </div>
          </div>
