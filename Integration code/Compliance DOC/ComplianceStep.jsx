...
// & Special Rates") from layer1_india.html. See complianceMachine.js's
// header comment for the NR-only eligibility gate and the mock-OCR upload
// pattern shared with the House Property step.
//
// The Approved Lower Rate field (comp-s197-rate) intentionally reproduces
// a real bug from the original rather than fixing it — see the long
// comment in deriveCompliance.js. Typing "0.05" here will store `0`,
...
function UploadStatus({ status, idleLabel, colorClass = 'text-brandGold' }) {
...
export default function ComplianceStep({ initialContext, onBack, onContinue }) {
  const [state, send] = useMachine(complianceMachine, { input: initialContext });
        ...
        <>
          ...
          <div className="p-5 bg-white/5 border border-white/10 rounded-2xl flex flex-col gap-4">
            <div className="flex items-center justify-between">
              ...
              <ToggleSwitch checked={form_10f.is_filed} onChange={(val) => send({ type: 'TOGGLE_10F_FILED', value: val })} />
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="flex items-center gap-3">
                      <label className="cursor-pointer bg-brandGold/10 hover:bg-brandGold/20 text-brandGold border border-brandGold/20 px-4 py-2 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all whitespace-nowrap">
                        ...
                        <input
                          ...
                          className="hidden"
                          onChange={(e) => {
                            if (e.target.files?.[0]) send({ type: 'START_10F_UPLOAD' });
                          }}
                        />
                      </label>
                  ...
                  <div className="p-4 bg-black/20 border border-white/5 rounded-xl flex flex-col gap-2 justify-center">
                    ...
                    <input
                      ...
                      onChange={(e) => send({ type: 'UPDATE_10F_ACK', value: e.target.value })}
          ...
          <div className="p-5 bg-white/5 border border-white/10 rounded-2xl flex flex-col gap-4">
            <div className="flex items-center justify-between">
              ...
              <ToggleSwitch checked={section_197_cert.is_available} onChange={(val) => send({ type: 'TOGGLE_S197_AVAILABLE', value: val })} />
            </div>
              ...
              <div id="div-s197-detail" className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-3 pt-4 border-t border-white/5">
                    ...
                    <div className="flex items-center gap-3">
                      <label className="cursor-pointer bg-brandGold/10 hover:bg-brandGold/20 text-brandGold border border-brandGold/20 px-4 py-2 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all whitespace-nowrap">
                        ...
                        <input
                          ...
                          className="hidden"
                          onChange={(e) => {
                            if (e.target.files?.[0]) send({ type: 'START_S197_UPLOAD' });
                          }}
                        />
                      </label>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => send({ type: 'UPDATE_S197_FIELD', field: 'rate', value: e.target.value })}
                    className={
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => send({ type: 'UPDATE_S197_FIELD', field: 'validity_start_date', value: e.target.value })}
                    className={
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => send({ type: 'UPDATE_S197_FIELD', field: 'validity_end_date', value: e.target.value })}
                    className={
                      'w-full bg-black/20 border rounded-xl text-xs text-white px-3 py-2.5 focus:border-brandGold focus:ring-0 focus:outline-none transition-all font-mono ' +
                      (section_197_cert._upload === 'success' ? 'ring-2 ring-green-500 bg-green-500/10 border-white/10' : 'border-white/10')
                    }
                    ...
                    {INCOME_TYPE_OPTIONS.map(([value, label]) => (
                      <label key={value} className="flex items-center gap-2 bg-black/20 border border-white/10 px-3 py-1.5 rounded-lg cursor-pointer hover:border-brandGold/50 transition-all">
                        <input
                          ...
                          onChange={(e) => send({ type: 'TOGGLE_S197_INCOME_TYPE', value, checked: e.target.checked })}
          ...
          <div className="p-5 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-between gap-4">
            ...
            <ToggleSwitch checked={chapter_xiia_elected} onChange={(val) => send({ type: 'TOGGLE_CHAPTER_XIIA', value: val })} />
          </div>
        </>
      )}
