// ────────────────────────────────────────────────────────────────────────
// ResidencySolverStep.jsx
// React port of panel-step-profile ("Screen 2A — Residency Detection" /
// the RS-001 Residency Solver) from layer1_india.html.
//
// - All Tailwind utility classes are copied verbatim from the source HTML
//   (brandGold / brandCyan / brandGreen / font-display come from the
//   tailwind.config.js shipped alongside this file; glass-card / scrollbar
//   / shimmer come from residencySolver.css).
// - Every onchange/onclick/oninput handler from the original <script> has
//   been replaced with an XState `send`. There is no direct DOM
//   manipulation anywhere in this file — all hidden/visible logic reads
//   from `context.ui.visibility` / `context.ui.tieBreaker`, which is
//   produced by deriveAll() in derive.js (the same functions the machine
//   uses), not by classList toggling.
// ────────────────────────────────────────────────────────────────────────
...
function YesNoSelect({ id, value, onChange, className = '' }) {
  return (
    <select
      id={id}
...
function QuestionRow({ id, title, subtitle, children }) {
  return (
...
export default function ResidencySolverStep({ initialContext, onBack, onContinue }) {
  const [state, send] = useMachine(residencyMachine, { input: initialContext });
  const ctx = state.context;
  const { residency_detail: rd, company_residency: cr, dtaa, entity_type, ui } = ctx;
  const { visibility: v, cert, tieBreaker: tb, showDualResidencyAlert, hasActiveLiveTrip } = ui;

  const updateField = (field, value) => send({ type: 'UPDATE_RESIDENCY_FIELD', field, value });
  const updateCompany = (field, value) => send({ type: 'UPDATE_COMPANY_RESIDENCY', field, value });
  ...
  return (
    <div id="panel-step-profile" className="glass-card p-6 lg:p-8 flex flex-col gap-6">
      ...
      {/* Live-derived residency badge */}
      <div className="p-4 bg-black/40 border border-white/10 rounded-2xl flex items-center justify-between">
        <div className="flex flex-col">
          <span className="text-[9px] font-black uppercase tracking-widest text-white/40">Derived Status</span>
          <span
            id="badge-lock-status"
            className={
              cert.status === 'ROR'
                ? 'text-3xl font-black font-display text-brandGold tracking-wider mt-1'
                : cert.status === 'RNOR'
                ? 'text-3xl font-black font-display text-amber-500 tracking-wider mt-1'
        ...
        <div className="text-right max-w-xs">
          <div id="cert-statutory-path" className="text-[10px] text-white/60">{cert.path}</div>
          <div id="cert-scope" className="text-[9px] text-white/30 mt-1">{cert.scope}</div>
        </div>
      </div>
