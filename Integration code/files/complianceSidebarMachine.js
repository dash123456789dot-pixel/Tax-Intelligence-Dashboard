 ...
 *
 * -----------------------------------------------------------------------
 * v2 PATCH (driven by extracting the Financial Life Snapshot step form —
 * layer1_india.html's updateBizEntityType()):
 *   - step-salary is additionally gated on entityType === 'individual'.
 *     Source quote (updateBizEntityType): `btnStepSalary.style.display =
 *     isInd ? '' : 'none'` plus `unlockedSteps.delete('step-salary')` when
 *     not individual. So step-salary now requires BOTH the "Salary" setup
 *     card AND entityType === 'individual' — previously this file only
 *     modeled the setup-card half of that rule.
 *   - Added `entityType` to context (default 'individual') and an
 *     "ENTITY_TYPE.SET" event, fed by financialSnapshotMachine.js via a
 *     `sendTo` action so the two machines never drift out of sync.
 * -----------------------------------------------------------------------
 * =============================================================================
 */
...
export const STEP_DEFINITIONS = [
  ...
  { id: "step-salary", phaseId: "phase-income", label: "Salary", gate: "isIndividualWithSalary" },
...
const GATES = {
  ...
  // source: isInd = (entityType === 'individual'); btnStepSalary shown/unlocked
  // only isInd, AND independently gated on the "Salary" setup-card checkbox.
  isIndividualWithSalary: (ctx) => ctx.entityType === "individual" && ctx.incomeHeads.salary,
