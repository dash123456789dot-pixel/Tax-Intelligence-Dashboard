 ...
 *
 * -----------------------------------------------------------------------
 * v2 PATCH: optional `actorRef` prop. Pass an already-started
 * complianceSidebarMachine actor (e.g. one also driven by
 * financialSnapshotMachine.js) to have this panel render THAT shared
 * actor's live state instead of spawning its own. Omit it and the panel
 * behaves exactly as before (self-contained, spawns its own machine).
 * -----------------------------------------------------------------------
 * =============================================================================
 */
...
export default function CompliancePanel({
  ...
  onStepChange,
  actorRef, // optional: share this panel's state with an externally-owned actor
}) {
  // Always spawn a local machine (stable hook order) — ignored when `actorRef` is passed.
  const [, , localActorRef] = useMachine(complianceSidebarMachine);
  const actor = actorRef || localActorRef;
  ...
  return (
    <>
      ...
      <aside className="l1-panel">
        ...
        {debugControls && <DebugControls context={context} send={send} />}
      </aside>
    </>
  );
}
