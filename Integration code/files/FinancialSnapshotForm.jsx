 ...
 * =============================================================================
 * GOAL
 *   Implement `FinancialSnapshotForm.jsx`: Step 1 of the Layer 1 India wizard
 *   — the "Financial Life Snapshot" form from layer1_india.html
 *   (<div id="panel-step-snapshot">) — as one React component, with its
 *   original dark/gold visual language retained in plain CSS, driven
 *   entirely by `financialSnapshotMachine.js`.
 *
 * DEPENDENCY / CONTRACT
 *   - `financialSnapshotMachine.js` (sibling file) owns every field's value
 *     and every derived show/hide rule (forcesOldRegime, showsCorporateBlock,
 *     showsMatProfitField, showsManufacturingWarning, showsMfgCommenceDateField,
 *     visibleCorporateRegimeRows, showsPresumptiveWarning, presumptiveWarningText).
 *     This component only renders what those functions return and sends
 *     events on input — it must never re-derive a show/hide rule itself.
 *   - `complianceSidebarMachine.js` + `CompliancePanel.jsx` (from the prior
 *     turn) are reused UNCHANGED for the live sidebar preview — this file
 *     spawns one shared actor with `useActorRef` and hands it to both
 *     `financialSnapshotMachine` (as `input.sidebarActorRef`, so the form's
 *     own sendTo-equivalent forwarding actions can reach it) and to
 *     `<CompliancePanel actorRef={...} />` (so the same live state renders
 *     in both places).
 *
 * REQUIREMENTS
 *   1. Retain the source's structure/labels exactly: "Tax Profile
 *      Configuration" bordered card (entity type + PAN + tax regime +
 *      Aadhaar-linked toggle + presumptive warning + corporate regime
 *      block + MAT profit box), then "Primary Income Heads" (Salary, House
 *      Property, Business & Trading) and "Wealth & International" (Capital
 *      Gains, Other Sources, International) setup-card grids, then a
 *      "Next Step →" button.
 *   2. No Tailwind arbitrary-value/peer/group-variant classes at runtime
 *      (no JIT compiler available) — plain scoped `.fs-*` CSS with the real
 *      px/hex/rgba values extracted from the source, injected via one
 *      <style> tag.
 *   3. Every field is a controlled input bound to `financialSnapshotMachine`
 *      context; every change sends an event, never local component state.
 *   4. The 6 "setup cards" (Salary/House Property/Business/Capital Gains/
 *      Other Sources/International) send "INCOME_HEAD.TOGGLE" /
 *      "INTERNATIONAL.TOGGLE" — the SAME event shapes complianceSidebarMachine
 *      already understands — so the machine's forwarding actions need no
 *      translation layer.
 *   5. `residencyStatus` flows ONE WAY, sidebar -> this form (via
 *      "RESIDENCY_STATUS.SYNC"), since Step 2 (not this file) owns it.
 *
 * ACCEPTANCE CRITERIA
 *   - Toggling the "Salary" setup card reveals "3. Salary" in the live
 *     Compliance Steps preview underneath/beside the form, and toggling
 *     entity type to anything but "Individual" hides it again even with
 *     the card still checked.
 *   - Selecting entity type "Partnership Firm" hides the Tax Regime
 *     dropdown, forces it to "Old Tax Regime", and reveals
 *     "12. Deductions (Ch VI-A)" in the sidebar preview — no other click
 *     required.
 *   - Selecting entity type "Company" (with sidebar residency status left
 *     at ROR) reveals the Corporate Tax Regime block and the MAT Profit
 *     field; checking "115BAA" hides the MAT field and unchecks any other
 *     corporate regime option.
 *   - Checking "International" reveals "9. Schedule FA & FSI" in the
 *     sidebar preview only while sidebar residency status is ROR/RNOR.
 * =============================================================================
 */
...
} from "./financialSnapshotMachine";
import { complianceSidebarMachine } from "./complianceSidebarMachine";
...
.fs-demo-layout { display: flex; flex-direction: column; gap: 24px; align-items: flex-start; }
.fs-demo-main { min-width: 0; width: 100%; }
@media (min-width: 1024px) { .fs-demo-layout { flex-direction: row; } .fs-demo-main { flex: 1; } }
`;

/* -------------------------------------------------------------------------
 * Small presentational pieces
 * ---------------------------------------------------------------------- */
function ToggleSwitch({ checked, onChange, size = "md", id }) {
  return (
    <button
      type="button"
      id={id}
...
function SetupCard({ icon, title, desc, checked, onToggle }) {
  return (
...
/* =============================================================================
 * THE FORM — pure function of (context, send). Reusable anywhere.
 * ========================================================================== */
export function FinancialSnapshotFields({ context, send, onNext }) {
...
/* =============================================================================
 * DEFAULT EXPORT — self-contained demo: spawns the shared sidebar actor,
 * wires this form's machine to it, and renders the live Compliance Steps
 * panel alongside the form so the binding is visibly provable.
 * ========================================================================== */
export default function FinancialSnapshotStep({
  sidebarActorRef: externalSidebarActorRef,
  showSidebarPreview = true,
  onNext,
}) {
  const localSidebarActorRef = useActorRef(complianceSidebarMachine);
  const sidebarActorRef = externalSidebarActorRef || localSidebarActorRef;

  const [snapState, snapSend] = useMachine(financialSnapshotMachine, {
    input: { sidebarActorRef },
  });

  // One-way sync: Step 2 (owned by the sidebar actor) tells this form its
  // residency status; this form never writes it back.
  ...
  return (
    <>
      <style>{FS_CSS}</style>
      <div className="fs-demo-layout">
        <div className="fs-demo-main">
          <FinancialSnapshotFields context={snapState.context} send={snapSend} onNext={onNext} />
        </div>
        {showSidebarPreview && <CompliancePanel actorRef={sidebarActorRef} debugControls />}
      </div>
    </>
  );
}
